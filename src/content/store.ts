/**
 * ============================================================================
 * STOREFRONT CONTENT
 * ============================================================================
 *
 * Every word and image on the storefront. Change the brand by changing this
 * file — no component needs to know which shop it is rendering.
 */

import { asset } from './asset';

export const brand = {
  name: 'Mejuri',
  /** The wordmark, drawn as a path so it stays sharp at any size. */
  wordmark: {
    viewBox: '0 0 817 170',
    path: 'M43.0637 0H0V168.085H34.2572V58.1758L75.66 168.085H106.111L147.271 58.1758V168.085H181.529V0H138.949L91.1277 124.988L43.0637 0ZM694.245 0H630.713V168.085H664.971V102.003H695.179C715.405 102.003 716.356 116.369 717.308 138.396C717.792 148.217 717.792 156.348 720.647 168.085H755.147C752.05 153.963 749.437 130.247 749.437 120.2C749.437 101.76 742.776 90.9813 734.921 85.7226C746.34 76.6158 752.05 63.6956 752.05 49.0863C752.05 14.853 726.599 0 694.245 0ZM698.519 69.6856H664.971V32.8055H698.034C710.647 32.8055 717.55 39.9969 717.55 51.2455C717.55 63.2254 710.647 69.6856 698.519 69.6856ZM782.258 168.085H817V0H782.258V168.085ZM436.33 106.304L436.572 0H402.072V115.899C402.072 130.978 395.411 139.841 383.283 139.841C370.428 139.841 365.67 126.677 365.67 115.655L332.606 115.411C332.606 148.461 348.541 170 383.041 170C421.104 170 436.33 146.058 436.33 106.304ZM599.311 98.6428V0H564.569V92.6703C564.569 122.603 556.005 138.396 534.118 138.396C509.135 138.396 503.425 118.998 503.425 92.6703V0H468.926V98.6428C468.926 145.1 491.764 170 534.118 170C580.037 170 599.311 141.739 599.311 98.6428ZM320.478 0H215.803V168.085H322.156V135.993H249.835V96.9712H316.931V66.0811H249.835V32.0742H320.495V0H320.478Z',
  },
} as const;

/** The rotating strip above the header. */
export const announcements = [
  'Complimentary Jewelry Cleaning at Stores',
  'Jewelry Under $200.',
  'Waived Piercing Fees.',
] as const;

/** Top-level navigation. */
export const navigation = [
  'All Jewelry',
  'Gifts',
  'New In',
  'Collections',
  'Jewelry Essentials',
  'Get Inspired',
] as const;

/**
 * The storefront sections, in scroll order.
 *
 *   kind: 'hero'     full-bleed opening section, plays video
 *   kind: 'campaign' full-bleed image section with a headline and a link
 *
 *   tone: 'light'    white type over a dark wash — for photography
 *   tone: 'dark'     ink type, no wash — for pale, white-background imagery
 */
export const sections = [
  {
    id: 'essentials',
    kind: 'hero',
    tone: 'dark',
    eyebrow: null,
    headline: 'THE ESSENTIALS',
    caption: 'Jewelry that becomes second skin. Worn every day, loved by generations.',
    cta: 'SHOP NOW',
    video: asset('/brand/media/hero-essentials.mp4'),
    poster: asset('/brand/media/hero-essentials.jpg'),
  },
  {
    id: 'solid-gold',
    kind: 'campaign',
    tone: 'light',
    eyebrow: null,
    headline: 'SOLID GOLD STAPLES',
    caption: null,
    cta: 'SHOP NOW',
    image: asset('/brand/media/solid-gold-staples.jpg'),
  },
  {
    id: 'sapphires',
    kind: 'campaign',
    tone: 'light',
    eyebrow: null,
    headline: 'LAB GROWN SAPPHIRES',
    caption: null,
    cta: 'SHOP NOW',
    image: asset('/brand/media/lab-grown-sapphires.jpg'),
  },
] as const;

/** The brand statement that sits between the campaign sections. */
export const statement = {
  heading: 'jewelry you can live in',
  body: [
    'For too long, fine jewelry was something you waited for. Reserved for birthdays. For anniversaries. Locked away in velvet boxes.',
    'We believe jewelry isn’t for waiting. It’s for living.',
  ],
} as const;
